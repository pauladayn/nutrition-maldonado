...
const CardsSection = ({ cardDetails }: { cardDetails: Array<CardDetail> }) => {
  return (
    <div className="flex max-w-full gap-4 flex-wrap md:mt-6 justify-center flex-wrap">
      {cardDetails.map(({ title, subtitle, path, img, btnText }: CardDetail, index: number) => (
        <div className="max-w-xs flex flex-col my-4 md:my-0" key={index}>
          <Image src={img} alt="card-image" className="rounded-2xl object-cover w-full h-52 shadow-lg" />
          <div className="mt-3 md:mt-6 flex flex-col gap-4 flex-1 rounded-2xl p-4 pb-8 bg-paleGreen shadow-xl">
            ...
            <div className="mt-auto">
              <Link
                href={`${path}/${index}`}
                className={`${poppins.className} self-start bg-primary text-neutral hover:bg-paleGreen hover:text-accent text-lg py-2 px-3 rounded-full shadow-lg`}>
                {btnText}
              </Link>
            </div>
          </div>
