...
const Footer = () => {
  return (
    <footer className="flex flex-col md:flex-row left-0 z-20 w-full p-4 bg-primary border-t shadow sm:flex md:items-center md:justify-between md:p-6">
        ...
        <ul className="flex flex-wrap items-center md: mt-2 text-sm font-medium text-neutral dark:text-neutral justify-center md:justify-start">
          {
            navLinks.map(({ title, href }) => (
              <li>
                <Link href={href} className="hover:underline me-4 md:me-6 cursor-pointer">{title}</Link>
              </li>
            ))
          }
        </ul>
        <p className="text-sm text-gray-500 text-center  md:text-start text-neutral dark:text-neutral mt-3">
          © 2024{" "}
          Nutri&Vita | Daiana Maldonado - Nutricionista - MP 194
        </p>
      </div>
      <ul className="flex flex-wrap items-center text-sm font-medium text-neutral dark:text-neutral sm:mt-0 justify-center space-x-6 order-1 md:order-2">
        {
          //hover:shadow-lg 
          //focus:shadow-lg
          //active:shadow-lg
